...
pub struct HyperSurface<A, B>(A, B);
pub struct HyperSurfaceSameSize<A, B>(A, B);
...
impl<L1, L2> Geometry for HyperSurfaceSameSize<L1, L2>
where
    L1: HeadTail,
    L2: HeadTail,
    //Scalar: Div<Scalar, Output = Scalar> + From<u16> + Copy,
    SimpleSurface<L1::Head, L2::Head>: GetLineAt,
    <SimpleSurface<L1::Head, L2::Head> as GetLineAt>::Scalar: Div<
            <SimpleSurface<L1::Head, L2::Head> as GetLineAt>::Scalar,
            Output = <SimpleSurface<L1::Head, L2::Head> as GetLineAt>::Scalar,
        > + From<u16>
        + Copy,
    <SimpleSurface<L1::Head, L2::Head> as GetLineAt>::Line: GetT<
        Scalar = <SimpleSurface<L1::Head, L2::Head> as GetLineAt>::Scalar,
        Value = Vector3<<SimpleSurface<L1::Head, L2::Head> as GetLineAt>::Scalar>,
    >,
    <<SimpleSurface<L1::Head, L2::Head> as GetLineAt>::Line as GetT>::Value: Copy,
    HyperSurface<L1::Tail, L2::Tail>: Geometry,
{
    fn polygonize(self, index: &mut GeoIndex, complexity: usize) -> anyhow::Result<()> {
        ...
        if let Some((f, s)) = rest_f.and_then(|f| rest_s.map(|s| (f, s))) {
            HyperSurface(f, s).polygonize(index, complexity)?;
        }
...
impl<A, B, L1, L2, Scalar, C, D> Geometry
...
impl<A, B, L1, L2, Scalar, C, D> Geometry
    for HyperSurfaceManyWithMany<
        HyperPath<L1, HyperPath<A, B, Scalar>, Scalar>,
        HyperPath<L2, HyperPath<C, D, Scalar>, Scalar>,
    >
where
    HyperPath<L1, HyperPath<A, B, Scalar>, Scalar>: Length<Scalar = Scalar>,
    HyperPath<L2, HyperPath<C, D, Scalar>, Scalar>: Length<Scalar = Scalar>,
    ...
    <SimpleSurface<A, L2> as GetLineAt>::Line: GetT<Scalar = Scalar, Value = Vector3<Scalar>>,
{
    fn polygonize(self, index: &mut GeoIndex, complexity: usize) -> anyhow::Result<()> {
        ...
        if f_l > s_l {
            ...
            let rest_s: HyperPath<L2, HyperPath<C, D, Scalar>, Scalar> = rest_s.push(s);
