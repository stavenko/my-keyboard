...
use super::primitives::{plane::Plane, Face};
...
impl Bsp {
    ...
    pub(crate) fn clip_by(mut self, mm: &Bsp) -> Self {
    ...
    pub fn build(faces: impl IntoIterator<Item = Polygon>) -> Option<Self> {
        ...
        face.and_then(|face| {
            ...
            let front = if front.is_empty() {
                ...
                Some(Box::new(Bsp::build(front)?))
            };
            let back = if back.is_empty() {
                None
            } else {
                Some(Box::new(Bsp::build(back)?))
            };
